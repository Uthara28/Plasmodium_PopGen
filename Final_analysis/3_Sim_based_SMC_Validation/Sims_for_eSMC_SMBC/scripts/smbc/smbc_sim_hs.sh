...
process_vcf() {
    ...
    # Print debugging information to log file
    {
        ...
        echo "Rscript --vanilla /data/proj2/home/students/u.srinivasan/Plasmodium_us/Inputs/msprime_sims/Sims_for_eSMC_SMBC/scripts/smbc/smbc_sim_hs.R \"$vcf_file\" \"$output_subdir\" \"$mut_rate\""
        echo "------------------------"
    } >> "$log_file"

    # Run the embedded R script with the VCF file and redirect both stdout and stderr to the log file
    Rscript --vanilla /data/proj2/home/students/u.srinivasan/Plasmodium_us/Inputs/msprime_sims/Sims_for_eSMC_SMBC/scripts/smbc/smbc_sim_hs.R \
...
find "$input_dir" -type f -name "*.vcf" | parallel --verbose -j 20 process_vcf {} "$input_dir" "$output_dir"
