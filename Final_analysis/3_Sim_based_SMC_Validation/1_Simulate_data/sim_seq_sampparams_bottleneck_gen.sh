...
SAMPLE_SIZE=$6     # Number of samples to simulate; can be adjusted as needed
...
run_simulation() {
  ...
  if [ "$MODEL" == "kingman" ]; then
    ...
    python3 /data/proj2/home/students/u.srinivasan/Plasmodium_us/Inputs/msprime_sims/scripts/sim_sequence_samplparams_bottleneck_esmc.py \
...
# If the model is Beta, loop over both mutation rates and alpha values
  ...
  parallel --verbose run_simulation ::: "${MUTATION_RATES_ARRAY[@]}" ::: "${ALPHAS_ARRAY[@]}"
fi
