...
# Ensure input and output directories are specified
...
output_dir_with_rate="${output_dir}/mut_rate_same_as_r"
mkdir -p "$output_dir_with_rate"

# Get the list of mhs files and calculate the correct array size
mhs_files=($(find "$input_dir" -type f -name "*.mhs"))
file_count=${#mhs_files[@]}

# Dynamically adjust SLURM array size based on file count
if [[ "$file_count" -eq 0 ]]; then
    echo "No mhs files found in input directory: $input_dir"
    exit 1
elif [[ "$SLURM_ARRAY_TASK_ID" -ge "$file_count" ]]; then
    echo "Array index exceeds number of files; exiting."
    exit 1
fi

# Get the mhs file corresponding to the current array task
mhs_file="${mhs_files[$SLURM_ARRAY_TASK_ID]}"
...
process_mhs() {
    ...
    # Extract mutation rate (assuming it follows '_m')
    mut_rate=$(echo "$prefix" | grep -oP '(?<=_r)[0-9.]+e[-+]?[0-9]+')

    # Extract recombination rate (assuming it follows '_r')
    recomb_rate=$(echo "$prefix" | grep -oP '(?<=_r)[0-9.]+e[-+]?[0-9]+')
    ...
    # Logging and command execution
    {
        ...
        echo "Rscript --vanilla /data/proj2/home/students/u.srinivasan/Plasmodium_us/Inputs/SMC/SMBC_regions.R \"$mhs_file\" \"$output_subdir\" \"$mut_rate\" \"$recomb_rate\""
        echo "------------------------"
    } >> "$log_file"

    # Execute the R script with the mhs file
    Rscript --vanilla /data/proj2/home/students/u.srinivasan/Plasmodium_us/Inputs/SMC/SMBC_regions.R \
